    ...
    <div className="w-full" ref={containerRef}>
      <div ref={ref} className="relative mx-auto md-pb-10 pb-4">
        {data.map((item, index) => (
          <div
            key={index}
            className="grid md:grid-cols-3 grid-cols-2 justify-start pt-10 md:pt-10 md:gap-6"
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block ml-7 custom-h2 text-sm font-proto leading-5! mb-0!">
                {item.title}
              </h3>
            </div>

            <div className="relative col-span-2 md:ml-0 ml-10">
              <h3 className="md:hidden block font-proto mb-4 text-left custom-h2 text-base!">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
